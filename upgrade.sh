...
#find px4 file in current dir
TMP_CMD="find $CUR_DIR -name *.px4"
RESULT=$(${TMP_CMD})

if [ ! -f "$RESULT" ];then
echo "px4 file is not exist,please copy px4 file to ${CURDIR}"
exit 0
fi

python px_uploader.py --port ${SERIAL_PORTS} ${RESULT}
